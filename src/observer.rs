...
use core_foundation::runloop::{CFRunLoopActivity, CFRunLoopObserver, CFRunLoopObserverContext, CFRunLoopObserverCreate, CFRunLoopObserverRef};
...
extern "C" fn observer_callback(_observer: CFRunLoopObserverRef, activity: CFRunLoopActivity, info: *mut c_void) {
...
pub fn create_oneshot_observer(interest: CFRunLoopActivity, tx: Sender<CFRunLoopActivity>) -> CFRunLoopObserver {
    let ctx = Box::into_raw(Box::new(CFRunLoopObserverContext {
        version: 0,
        info: Box::into_raw(Box::new(ObserverContextInfo {
            interest,
            tx,
        })) as *mut c_void,
        retain: None,
